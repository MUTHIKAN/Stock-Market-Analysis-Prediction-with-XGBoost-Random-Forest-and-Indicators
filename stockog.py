...
if st.session_state.stock_data is not None:
    ...
    if open_price_input > 0:
        ...
        last_row['Open'] = open_price_input  # Set the Open Price manually

        # Create lag features based on the new Open Price
        for lag in range(1, 6):
            last_row[f'Close_Lag_{lag}'] = last_row['Close'].shift(lag).iloc[0]
        
        # Prepare the feature set for prediction
        last_row_features = last_row[['Open', '5_day_MA', '10_day_MA', '50_day_MA', 'RSI', 'Bollinger_High', 'Bollinger_Low', 'MACD', 'Signal'] + [f'Close_Lag_{i}' for i in range(1, 6)]]

        # Predict High, Low, and Close prices using the trained models
        predicted_high = st.session_state.model_high.predict(last_row_features)[0]
        predicted_low = st.session_state.model_low.predict(last_row_features)[0]
        predicted_close = st.session_state.model_close.predict(last_row_features)[0]
